// Script to populate Latitude/Longitude columns in Coda Activities table
// Run with: node update-coordinates.mjs
...
// Convert to MCP-friendly JSON array format for easy copy-paste
const rowIds = Object.keys(ACTIVITY_LATLNG);
const LAT_COL = 'c-1oOmaseFGM';
const LNG_COL = 'c-tmpeKQQks2';

// Split into batches of 100
const batch1 = rowIds.slice(0, 100).map(rowId => {
  const [lat, lng] = ACTIVITY_LATLNG[rowId];
  return {
    rowNumberOrId: rowId,
    updateCells: [
      { columnId: LAT_COL, value: lat },
      { columnId: LNG_COL, value: lng }
    ]
  };
});

const batch2 = rowIds.slice(100).map(rowId => {
  const [lat, lng] = ACTIVITY_LATLNG[rowId];
  return {
    rowNumberOrId: rowId,
    updateCells: [
...
console.log('=== BATCH 1 (100 rows) ===');
console.log(JSON.stringify(batch1, null, 2));
console.log('\n\n=== BATCH 2 (' + batch2.length + ' rows) ===');
