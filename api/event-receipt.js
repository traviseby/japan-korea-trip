// Resolve a hosted URL for an event's Receipt attachment from Coda
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const colsResp = await fetch(
      `https://coda.io/apis/v1/docs/${docId}/tables/${eventsTable.id}/columns`,
      {
        headers: {
          Authorization: `Bearer ${CODA_TOKEN}`,
          'Content-Type': 'application/json'
    ...
    const rowResp = await fetch(
      `https://coda.io/apis/v1/docs/${docId}/tables/${eventsTable.id}/rows/${encodeURIComponent(rowId)}?valueFormat=rich&useColumnNames=false`,
      {
        headers: {
          Authorization: `Bearer ${CODA_TOKEN}`,
          'Content-Type': 'application/json'
    ...
    console.error('Error resolving event receipt:', error);
