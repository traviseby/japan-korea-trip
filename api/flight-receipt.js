// Resolve a hosted URL for a flight's Receipt attachment from Coda
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const rowResp = await fetch(
      `https://coda.io/apis/v1/docs/${docId}/tables/${flightsTable.id}/rows/${encodeURIComponent(rowId)}?valueFormat=rich&useColumnNames=false`,
      {
        headers: {
          Authorization: `Bearer ${CODA_TOKEN}`,
          'Content-Type': 'application/json'
    ...
    console.error('Error resolving flight receipt:', error);
